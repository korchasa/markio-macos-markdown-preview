...
6. While compared, check `View ▸ Compare Side by Side` → unchanged content
   stays single-column at full width; each changed region becomes a
   two-column row — v1 blocks left marked red, v2 blocks right marked
   green; one scroll moves everything. Resize the window and move the
   width slider — the layout holds. Uncheck → the inline view returns.
   Quit and relaunch while the toggle is on → a new compare opens in
   split layout (persisted preference).
