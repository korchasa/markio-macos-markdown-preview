...
- **Desc:** The `.app` bundle ships a Markio app icon (document-with-text-lines design). Source PNGs live in `packaging/Assets.xcassets`; `deno task app` compiles the catalog to `Assets.car` via `actool` and `Info.plist` references it by name (`CFBundleIconName`). The loose `AppIcon.icns` actool also emits is deleted: it caps at 256×256 and App Store ingest would pick it over the catalog's 1024×1024. Shown in Finder, Dock, and the app switcher.
...
- **Desc:** Minimal read-only native menu surface. File has exactly three ordered groups: `Open…`/`Open Recent`; `Copy File Path`/[REF:fr:compare | Compare…/Stop Comparing]; `Close`/`Close All`. New, document-write, share, import/export, and print commands are absent; no blank items or stray separators remain. Close uses ⌘W; Close All uses ⌥⌘W and targets documents only. `Copy File Path` writes the focused document's exact absolute filesystem path ([REF:fr:multidoc | FR-MULTIDOC]) to `NSPasteboard` as plain text: no `file://`, percent encoding, trailing newline, or symlink resolution; it is disabled without a focused file-backed document and has no shortcut. Edit retains standard editing commands, including ordinary Copy, plus [REF:fr:find | Find/Find Next/Find Previous]; it contains no `Copy File Path`. The English-only bundle uses English custom titles. This requirement does not alter other menus.
- **Tasks:** [REF:task:2026-07-copy-file-path | copy-file-path]; [REF:task:2026-07-reorder-file-menu | reorder-file-menu]
- **Scenario:** With `/repo/notes.md` focused, File ▸ Copy File Path puts `/repo/notes.md` on the clipboard; switching documents targets the newly focused window. Without a focused file-backed document, Copy File Path, Close, and Close All remain visible but disabled and the clipboard stays unchanged. Edit ▸ Copy still copies selected preview text.
- **Acceptance:** `Tests/MarkioTests/FilePathCopyTests.swift::testCopiesAbsoluteFilePathAsPlainText`; `Tests/MarkioTests/FilePathCopyTests.swift::testUnavailableFileLeavesPasteboardUntouched`; `Tests/MarkioTests/MenuArtifactCleanerTests.swift::testRemovesNativeNewAndPreservesOpenCommands`; `Tests/MarkioTests/MenuArtifactCleanerTests.swift::testForwardsMenuUpdateBeforeRemovingNativeNew`; `Tests/MarkioTests/MenuArtifactCleanerTests.swift::testInstallerIsIdempotentAndRepairsDelegateReplacement`; `manual — maintainer — documents/checklists/menu.md`
- **Status:** [x]
...
### 3.21 FR-COMPARE: Inline-diff compare [ANC:fr:compare]
- **Desc:** Read-only comparison of the open document against a BASELINE version, rendered as an inline diff in the SAME window — one window, one scroll; no second window, tiling, or scroll mirroring (v3 decision replacing side-by-side; still no editing or merging). `File ▸ Compare…` prompts for the baseline via a powerbox `NSOpenPanel` (the "Compare" click is the sandbox grant, as in [REF:fr:local-links | FR-LOCAL-LINKS]); the baseline is READ, never opened as a window. The current document renders through the normal pipeline (GFM, Mermaid, KaTeX, highlight, sanitize); a vendored line diff (Markdown source, grouped into top-level blocks) then annotates it: blocks ADDED since the baseline are tinted with an "added" accent; runs REMOVED from the baseline are rendered standalone and inserted at their position as dimmed "removed" blocks (context-dependent constructs best-effort). Block granularity v1; word-level refinement deferred. TOC, find, and line width work over the diff view. Two layouts, toggled by View ▸ `Compare Side by Side` (checkmark — a view concern, so it sits beside the TOC toggle, not in File; enabled only while compared; a global reading preference persisted like the TOC visibility, default inline): INLINE (annotations in the single rendered document) and SPLIT — interleaved: unchanged content renders ONCE at full width; each changed region is a two-column row, baseline blocks left (removed marks), current blocks right (added marks). Both versions are still rendered whole-text full-fidelity, then finished blocks are regrouped into rows — never chunk-wise rendering, no scroll mirroring or vertical-alignment machinery of any kind; TOC/heading ids come from the current document's render, find covers both sides. Live reload re-diffs (baseline re-read; unreadable baseline logs and exits compare). `Stop Comparing` restores the plain render. Picking the document's own file is a no-op; the baseline choice is session-only.
- **Tasks:** [REF:task:2026-07-side-by-side-compare | side-by-side-compare]; [REF:task:2026-07-reorder-file-menu | reorder-file-menu]; [REF:task:2026-07-compare-absolute-scroll-sync | compare-absolute-scroll-sync]; [REF:task:2026-07-inline-diff-compare | inline-diff-compare]; [REF:task:2026-07-split-compare-view | split-compare-view]
- **Menu placement:** Follows [REF:fr:menu | FR-MENU].
- **Scenario:** Reading `spec-v2.md`, the user picks File ▸ Compare… and chooses `spec-v1.md` → the same window now shows v2 with the sections new since v1 tinted green and the paragraphs deleted since v1 inlined as dimmed red blocks at their original positions; find and the TOC cover the whole diff view; Stop Comparing returns the plain document.
- **Acceptance:** `Tests/MarkioTests/CompareTests.swift::testDiffMarksAddedBlocks`; `Tests/MarkioTests/CompareTests.swift::testDiffInsertsRemovedBlocks`; `Tests/MarkioTests/CompareTests.swift::testIdenticalDocumentsShowNoMarkers`; `Tests/MarkioTests/CompareTests.swift::testStopComparingRestoresPlainRender`; `Tests/MarkioTests/CompareTests.swift::testSelfCompareIsNoOp`; `Tests/MarkioTests/CompareTests.swift::testSplitLayoutShowsBothVersionsWithMarks`; `Tests/MarkioTests/CompareTests.swift::testSplitKeepsUnchangedFullWidth`; `Tests/MarkioTests/CompareTests.swift::testSplitToggleSwitchesLayoutLive`; `Tests/MarkioTests/CompareTests.swift::testSplitPreferencePersists`; `manual — maintainer — documents/checklists/compare.md`
- **Status:** [x]

### 3.22 FR-MERMAID-ZOOM: Mermaid zoom & copy [ANC:fr:mermaid-zoom]
- **Desc:** Rendered Mermaid diagrams ([REF:fr:mermaid | FR-MERMAID]) are interactive. (a) **Zoom:** clicking a diagram opens an in-page lightbox overlay showing it at natural size — wide flowcharts are unreadable at reading-column width; scroll wheel (and trackpad pinch) zooms around the cursor, dragging pans, `+`/`−`/reset buttons step the scale; Esc, the ✕ button, or a backdrop click closes; a re-render ([REF:fr:live-reload | FR-LIVE-RELOAD]) closes a stale overlay. (b) **Copy PNG:** a hover button on the diagram (and one in the overlay) copies the diagram to the clipboard as a raster PNG at 2× (longest side ≤ 8192 px, canvas filled with the page background). The page re-renders the diagram source off-screen with `htmlLabels: false` — labels become SVG `<text>`, because WebKit does not paint `foreignObject` when an SVG is drawn to a canvas — rasterizes via canvas, and posts base64 over the one-way `markioCopyImage` handler; the native shell writes `NSPasteboard` `.png` (in-page `navigator.clipboard` is unreliable in the sandboxed `WKWebView`, as in [REF:fr:code-copy | FR-CODE-COPY]). Hover/overlay UI text is never a find match ([REF:fr:find | FR-FIND]); the handler post is guarded, so render-only hosts (Quick Look) degrade to a no-op. Raster failures (malformed source, decode errors) flash "Failed" on the button and log — never crash (NFR Reliability).
- **Tasks:** [REF:task:2026-07-mermaid-zoom-copy | mermaid-zoom-copy]
- **Scenario:** Reading an architecture doc, the user clicks a wide flowchart → the lightbox opens; wheel-zooming into a cluster makes labels readable; Esc returns to reading. Hovering the diagram and clicking Copy PNG puts a crisp raster on the clipboard, pasted straight into a chat.
- **Acceptance:** `Tests/MarkioTests/MermaidZoomTests.swift::testClickOpensZoomOverlay`; `Tests/MarkioTests/MermaidZoomTests.swift::testZoomAndPanTransform`; `Tests/MarkioTests/MermaidZoomTests.swift::testOverlayCloses`; `Tests/MarkioTests/MermaidZoomTests.swift::testRerenderClosesOverlay`; `Tests/MarkioTests/MermaidZoomTests.swift::testRasterSVGCarriesTextLabelsNoForeignObject`; `Tests/MarkioTests/MermaidZoomTests.swift::testCopyPNGWritesRasterToPasteboard`; `Tests/MarkioTests/MermaidZoomTests.swift::testOverlayCopyPNGButtonCopies`; `Tests/MarkioTests/MermaidZoomTests.swift::testFindSkipsMermaidUI`
- **Status:** [x]
...
- **Sec:** No network; the JS bridge is native→web calls plus five one-way page→native message handlers, each carrying a single string: `markioTOC` (current heading id — [REF:fr:toc | FR-TOC]), `markioCopy` (raw code text → `NSPasteboard` — [REF:fr:code-copy | FR-CODE-COPY]), `markioScroll` (debounced scroll offset → per-document position store — [REF:fr:session-restore | FR-SESSION-RESTORE]), `markioLink` (raw clicked href → default-deny native resolver — [REF:fr:local-links | FR-LOCAL-LINKS]), and `markioCopyImage` (base64 diagram PNG → `NSPasteboard` — [REF:fr:mermaid-zoom | FR-MERMAID-ZOOM]); `WKWebView` confined to bundled file URLs. Raw inline HTML passes a DOMPurify allowlist sanitizer before DOM insertion ([REF:fr:inline-html | FR-INLINE-HTML]).
...
## 5. Interfaces
- **UI:** Native document windows (`DocumentGroup`) + a bottom bar (line-width control). File/Edit structure follows [REF:fr:menu | FR-MENU]. State restoration; one window per document, no window tabs. Drag a file onto a window → opens it in a new window. Preview surface = `WKWebView`.
- **Proto (internal):** Native → web view (via `callAsyncJavaScript`): set Markdown source (`render`), set reading width (`setContentWidth`, `ch`), set appearance (`setDark`), find text (`search`/`findNext`/`findPrev`/`clearSearch`), read the outline (`getOutline`), jump to a heading (`scrollToHeading`), read the current section (`getCurrentSection`), restore the reading position (`setScrollY`), render an inline diff against a baseline (`renderDiff` — [REF:fr:compare | FR-COMPARE]). Web → native: five one-way `WKScriptMessageHandler`s — `markioTOC` (the page pushes the current heading id when it changes on scroll — [REF:fr:toc | FR-TOC]), `markioCopy` (a copy-button click pushes the block's raw code text, written to `NSPasteboard` — [REF:fr:code-copy | FR-CODE-COPY]), `markioScroll` (the page pushes the debounced scroll offset, persisted per document — [REF:fr:session-restore | FR-SESSION-RESTORE]), `markioLink` (a click on a relative Markdown link pushes the raw href; native code resolves and opens it — [REF:fr:local-links | FR-LOCAL-LINKS]), and `markioCopyImage` (a diagram Copy-PNG click pushes the base64 raster, written to `NSPasteboard` as `.png` — [REF:fr:mermaid-zoom | FR-MERMAID-ZOOM]); external link clicks are intercepted by the `WKNavigationDelegate` (open in the default browser via `NSWorkspace`), `#anchor` clicks scroll in-page; width changes persist natively (slider → `UserDefaults`).
