...
- **Desc:** Parallel READING of two documents (spec v1 vs v2, agent report before/after) — NOT a diff editor (no content comparison or difference highlighting). File groups `Compare Side by Side…`/`Stop Comparing` with other focused-document actions between Open Recent and Close. Compare prompts for the second file via a powerbox `NSOpenPanel` (the "Compare" click is the sandbox grant, as in [REF:fr:local-links | FR-LOCAL-LINKS]), opens it through `NSDocumentController` (one window per document preserved — an already-open file just focuses, [REF:fr:multidoc | FR-MULTIDOC]), tiles both windows left/right on the initiator's screen, and links them: scrolling either window scrolls the other to the same fraction of ITS OWN scrollable height (`scrollY / (scrollHeight − viewport)` equal on both sides), live in both directions, with one-shot echo suppression on the page so mirroring never feeds back. Any navigation mirrors — TOC jumps and find included ("paired reading follows any navigation", a deliberate decision). `Stop Comparing` unlinks; closing either window drops the pair (weak, symmetric peers — the other window stays open and independent). Per-window state (TOC, find, width, scroll persistence) is untouched; pairs are session-only (not persisted); self-pairing is impossible by construction (one file = one window).
- **Tasks:** [REF:task:2026-07-side-by-side-compare | side-by-side-compare]; [REF:task:2026-07-reorder-file-menu | reorder-file-menu]
- **Menu placement:** Follows [REF:fr:menu | FR-MENU].
- **Scenario:** Reading `spec-v2.md`, the user picks File ▸ Compare Side by Side… and chooses `spec-v1.md` → the two windows tile left/right; scrolling v2 to its middle puts v1 at its own middle; clicking a TOC entry in v2 moves v1 alongside; Stop Comparing (or closing v1) leaves both windows fully independent.
- **Acceptance:** `Tests/MarkioTests/CompareTests.swift::testScrollMirrorsToLinkedPeerProportionally`; `Tests/MarkioTests/CompareTests.swift::testNoFeedbackLoopBetweenLinkedPeers`; `Tests/MarkioTests/CompareTests.swift::testUnlinkStopsMirroring`; `Tests/MarkioTests/CompareTests.swift::testDeallocatedPeerDropsPair`; `manual — maintainer — documents/checklists/compare.md` (menu, panel, tiling in a real `.app`)
- **Status:** [ ]
