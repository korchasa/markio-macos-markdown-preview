...
- **Sec:** No network; the JS bridge is native→web calls plus five one-way page→native message handlers, each carrying a single string: `markioTOC` (current heading id — [REF:fr:toc | FR-TOC]), `markioCopy` (raw code text → `NSPasteboard` — [REF:fr:code-copy | FR-CODE-COPY]), `markioScroll` (debounced scroll offset → per-document position store — [REF:fr:session-restore | FR-SESSION-RESTORE]), `markioLink` (raw clicked href → default-deny native resolver — [REF:fr:local-links | FR-LOCAL-LINKS]), and `markioSyncScroll` (live scroll fraction 0…1 while a compare pair is active — [REF:fr:compare | FR-COMPARE]); `WKWebView` confined to bundled file URLs. Raw inline HTML passes a DOMPurify allowlist sanitizer before DOM insertion ([REF:fr:inline-html | FR-INLINE-HTML]).
