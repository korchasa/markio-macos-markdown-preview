...
### 3.21 FR-COMPARE: Side-by-side compare [ANC:fr:compare]
- **Desc:** Parallel READING of two documents (spec v1 vs v2, agent report before/after) — NOT a diff editor (no content comparison or difference highlighting). File groups `Compare Side by Side…`/`Stop Comparing` with other focused-document actions between Open Recent and Close. Compare prompts for the second file via a powerbox `NSOpenPanel` (the "Compare" click is the sandbox grant, as in [REF:fr:local-links | FR-LOCAL-LINKS]), opens it through `NSDocumentController` (one window per document preserved — an already-open file just focuses, [REF:fr:multidoc | FR-MULTIDOC]), tiles both windows left/right on the initiator's screen, and links them: scrolling either window scrolls the other by the SAME ABSOLUTE PIXEL DISTANCE (delta mirroring), live in both directions, with one-shot echo suppression on the page so mirroring never feeds back. Each window clamps within its own bounds: a shorter document stops at its edge while the longer one continues, and the moment scrolling reverses the stopped peer picks up again immediately (post-clamp alignment is deliberately path-dependent). Linking seeds the peer to the initiator's absolute offset, clamped. Any navigation mirrors — TOC jumps and find included ("paired reading follows any navigation", a deliberate decision). `Stop Comparing` unlinks; closing either window drops the pair (weak, symmetric peers — the other window stays open and independent). Per-window state (TOC, find, width, scroll persistence) is untouched; pairs are session-only (not persisted); self-pairing is impossible by construction (one file = one window).
- **Tasks:** [REF:task:2026-07-side-by-side-compare | side-by-side-compare]; [REF:task:2026-07-reorder-file-menu | reorder-file-menu]; [REF:task:2026-07-compare-absolute-scroll-sync | compare-absolute-scroll-sync]
- **Menu placement:** Follows [REF:fr:menu | FR-MENU].
- **Scenario:** Reading `spec-v2.md`, the user picks File ▸ Compare Side by Side… and chooses the shorter `spec-v1.md` → the two windows tile left/right; scrolling v2 down 500 px moves v1 down 500 px; when v1 hits its end it stays while v2 continues, and the first upward scroll of v2 pulls v1 back up immediately; clicking a TOC entry in v2 moves v1 by the same distance; Stop Comparing (or closing v1) leaves both windows fully independent.
- **Acceptance:** `Tests/MarkioTests/CompareTests.swift::testScrollMirrorsSameAbsoluteDistanceToLinkedPeer`; `Tests/MarkioTests/CompareTests.swift::testClampedPeerPicksUpImmediatelyOnReverseScroll`; `Tests/MarkioTests/CompareTests.swift::testOnePixelStepScrollingMovesPeer`; `Tests/MarkioTests/CompareTests.swift::testLinkSeedsPeerToInitiatorsAbsoluteOffset`; `Tests/MarkioTests/CompareTests.swift::testNoFeedbackLoopBetweenLinkedPeers`; `Tests/MarkioTests/CompareTests.swift::testUnlinkStopsMirroring`; `Tests/MarkioTests/CompareTests.swift::testDeallocatedPeerDropsPair`; `manual — maintainer — documents/checklists/compare.md` (menu, panel, tiling in a real `.app`)
- **Status:** [ ]
...
- **Sec:** No network; the JS bridge is native→web calls plus six one-way page→native message handlers, each carrying a single string: `markioTOC` (current heading id — [REF:fr:toc | FR-TOC]), `markioCopy` (raw code text → `NSPasteboard` — [REF:fr:code-copy | FR-CODE-COPY]), `markioScroll` (debounced scroll offset → per-document position store — [REF:fr:session-restore | FR-SESSION-RESTORE]), `markioLink` (raw clicked href → default-deny native resolver — [REF:fr:local-links | FR-LOCAL-LINKS]), `markioSyncScroll` (live scroll fraction 0…1 while a compare pair is active — [REF:fr:compare | FR-COMPARE]), and `markioCopyImage` (base64 diagram PNG → `NSPasteboard` — [REF:fr:mermaid-zoom | FR-MERMAID-ZOOM]); `WKWebView` confined to bundled file URLs. Raw inline HTML passes a DOMPurify allowlist sanitizer before DOM insertion ([REF:fr:inline-html | FR-INLINE-HTML]).
...
- **Proto (internal):** Native → web view (via `callAsyncJavaScript`): set Markdown source (`render`), set reading width (`setContentWidth`, `ch`), set appearance (`setDark`), find text (`search`/`findNext`/`findPrev`/`clearSearch`), read the outline (`getOutline`), jump to a heading (`scrollToHeading`), read the current section (`getCurrentSection`), restore the reading position (`setScrollY`), drive compare mirroring (`setCompareSync`, `setScrollFraction`, `getScrollFraction` — [REF:fr:compare | FR-COMPARE]). Web → native: six one-way `WKScriptMessageHandler`s — `markioTOC` (the page pushes the current heading id when it changes on scroll — [REF:fr:toc | FR-TOC]), `markioCopy` (a copy-button click pushes the block's raw code text, written to `NSPasteboard` — [REF:fr:code-copy | FR-CODE-COPY]), `markioScroll` (the page pushes the debounced scroll offset, persisted per document — [REF:fr:session-restore | FR-SESSION-RESTORE]), `markioLink` (a click on a relative Markdown link pushes the raw href; native code resolves and opens it — [REF:fr:local-links | FR-LOCAL-LINKS]), `markioSyncScroll` (the page pushes the live scroll fraction while compare sync is enabled — [REF:fr:compare | FR-COMPARE]), and `markioCopyImage` (a diagram Copy-PNG click pushes the base64 raster, written to `NSPasteboard` as `.png` — [REF:fr:mermaid-zoom | FR-MERMAID-ZOOM]); external link clicks are intercepted by the `WKNavigationDelegate` (open in the default browser via `NSWorkspace`), `#anchor` clicks scroll in-page; width changes persist natively (slider → `UserDefaults`).
