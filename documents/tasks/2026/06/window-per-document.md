...
- `MarkviewApp` = `WindowGroup` + one `@StateObject AppModel` (singleton) + `AppDelegate` bridging Finder/argv opens into that single model.
...
- [x] FR-MULTIDOC: `MarkdownDocument` decodes UTF-8 file contents and fails fast on non-UTF-8.
  - Test: `Tests/MarkviewTests/DocumentReadTests.swift::testDecodesUTF8` / `::testRejectsInvalidUTF8`
...
- [x] FR-LIVE-RELOAD preserved per-window: external edit refreshes that window.
  - Test: `Tests/MarkviewTests/LiveReloadTests.swift::testPreviewUpdatesWhenFileChanges`
...
3. `MarkviewApp`: `DocumentGroup(viewing: MarkdownDocument.self) { config in ContentView(document:config.document, fileURL:config.fileURL) }`. Drop custom "Open…" command (DocumentGroup provides File ▸ Open / Open Recent).
...
- **`.app` packaging:** `packaging/Info.plist` + `make app`/`make prod` build `Markview.app` (under `.build/`) → single instance, all opens become windows in one process, Finder "Open With" works. `make dev` stays the raw binary. Verified live: one PID, two windows, full-path titles.
